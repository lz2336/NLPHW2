...
class FeatureExtractor(object):
    ...
    @staticmethod
    def get_intervening_POS(idx1, idx2, tokens):
        if idx1 < idx2:
            i, j = idx1, idx2
        else:
            i, j = idx2, idx1
        ...
        for token_idx in range(i, j + 1):
            ...
            if 'tag' in token:
                ...
                if token['tag'] == 'PO':
                    num_intervening_PO += 1
                if token['tag'] == 'NN':
                    num_intervening_NN += 1
        return str(num_intervening_NN), str(num_intervening_VV), str(num_intervening_PO)
    ...
    def extract_features(tokens, buffer, stack, arcs):
        ...
        if buffer:
            ...
            if stack:
                ...
                num_intervening_NN, num_intervening_VV, num_intervening_PO = FeatureExtractor.get_intervening_POS(stack_idx0, buffer_idx0, tokens)
                result.append('STK_BUF_DIST_0_' + word_distance_0)
                result.append('STK_BUF_INTV_NN_' + num_intervening_NN)
                result.append('STK_BUF_INTV_VV_' + num_intervening_VV)
                result.append('STK_BUF_INTV_PO_' + num_intervening_PO)
